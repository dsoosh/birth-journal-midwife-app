#!/usr/bin/env python3
"""Deploy Flutter midwife app to Chrome for web debugging."""
...
def get_local_wifi_ip():
    ...
    try:
        # Connect to a remote host (doesn't actually send data)
...
def main():
    ...
    print(f"\n{CYAN}============================================{RESET}")
    print(f"{CYAN}  Deploying Polozne Midwife App to Chrome{RESET}")
    print(f"{CYAN}============================================{RESET}")
    print()
    print(f"Backend API: {YELLOW}{api_url}{RESET}")
    print(f"Target: {YELLOW}Chrome (Web){RESET}")
    print()
    ...
    print(f"\n{GREEN}Building and launching app in Chrome...{RESET}")
    try:
        subprocess.run(
            f"flutter run -d chrome --dart-define API_BASE_URL={api_url}",
            shell=True,
            check=True,
        )
